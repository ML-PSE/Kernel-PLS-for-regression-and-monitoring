...
for n_comp in range(1,12):
    print(n_comp)
    K = Kernel(X_train_scaled, 1)
    K_t = Kernel_test(X_train_scaled, X_val_scaled, 1)
...
plt.ylabel('y predicted'), plt.ylabel('y actual'), plt.title('NOC data')
plt.grid()
...
plt.ylabel('y predicted'), plt.ylabel('y actual'), plt.title('faulty data')
plt.grid()
